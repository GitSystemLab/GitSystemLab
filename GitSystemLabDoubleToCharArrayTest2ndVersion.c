	#include "GitSystemLabDoubleToCharArray2ndVersion.h"
	
	int main(void){
		// This sample file should be in the same directory as the Header file, GitSystemLabDoubleToCharArray.h , is included in.
		double Value0 = (double)0.0;                     // "double" value which is converted to a char array.
		...
		int LanguageKind0 = 0;                           // 1: Normal Mode, 2: Special Mode. Otherwise: Output is "Null".
		int i = 0;
		
		for(i=1;i<=100;i++){
			//Sample//
			Value0 = (double)-99899.99951;
